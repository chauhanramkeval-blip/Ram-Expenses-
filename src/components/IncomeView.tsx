...
export const IncomeView: React.FC<IncomeViewProps> = ({
  ...
  const [viewTab, setViewTab] = useState<"list" | "charts">(initialViewTab);
  ...
  // Export to Excel / CSV for Income with universal mobile support
  const handleExportIncomeCSV = async () => {
    ...
    try {
      ...
      alert("Failed to export income: " + (err?.message || "Check device permissions"));
    }
  };
    ...
    <div id="khata-income-section-container" className="space-y-5 animate-fadeIn">
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#F1F3F4] pb-5">
          ...
          {/* Action Buttons: Add Income, Charts View & Categories Manual Edit */}
          <div className="flex items-center gap-2 flex-wrap self-start md:self-auto">
            <button
              id="btn-toggle-income-charts-header"
              onClick={() => setViewTab(viewTab === "charts" ? "list" : "charts")}
              className={`flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold rounded-full border transition-all cursor-pointer ${
                viewTab === "charts"
                  ...
                  : "text-[#0F9D58] bg-[#E6F4EA] hover:bg-[#CEEAD6] border-[#CEEAD6]"
              }`}
            >
              <PieChart size={14} />
              <span>{viewTab === "charts" ? "Show Logs List" : "Income Charts"}</span>
            </button>
      ...
      {/* 2. PRIMARY INCOME SECTION NAVIGATION BAR (Records List vs Visual Charts) */}
      <div
        id="income-view-mode-navbar"
        className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-2 sm:p-2.5 rounded-3xl border border-[#E8EAED] shadow-xs"
      >
        <div className="grid grid-cols-2 p-1 bg-[#F1F3F4] rounded-2xl w-full sm:w-80">
          <button
            type="button"
            id="nav-btn-income-list"
            ...
            className={`flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs sm:text-sm font-bold transition-all cursor-pointer ${
              viewTab === "list"
                ? "bg-white text-[#202124] shadow-xs font-extrabold"
                : "text-[#5F6368] hover:text-[#202124]"
            }`}
          >
            <Wallet size={16} className={viewTab === "list" ? "text-[#0F9D58]" : "text-[#5F6368]"} />
            <span>Inflow Records</span>
            <span
              className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${
                viewTab === "list" ? "bg-[#E6F4EA] text-[#0F9D58]" : "bg-[#E8EAED] text-[#5F6368]"
              }`}
            >
              {incomes.length}
            </span>
          ...
          >
            <PieChart size={16} className={viewTab === "charts" ? "text-[#0F9D58]" : "text-[#5F6368]"} />
            <span>Visual Charts</span>
            <span
              className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${
                viewTab === "charts" ? "bg-[#0F9D58] text-white" : "bg-[#E6F4EA] text-[#0F9D58]"
              }`}
            >
              Charts
            </span>
        ...
        {/* Right Info / Quick Switch Helper */}
        <div className="flex items-center gap-2 text-xs text-[#5F6368] px-2 sm:px-0 justify-between sm:justify-end">
          {viewTab === "list" ? (
            <button
              onClick={() => setViewTab("charts")}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#E6F4EA] hover:bg-[#CEEAD6] text-[#0F9D58] font-bold rounded-full transition-colors cursor-pointer border border-[#CEEAD6]"
            >
              <BarChart3 size={13} />
            ...
            <button
              onClick={() => setViewTab("list")}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#F1F3F4] hover:bg-[#E8EAED] text-[#3C4043] font-semibold rounded-full transition-colors cursor-pointer border border-[#DADCE0]"
            >
              <Wallet size={13} />
      ...
      {/* 3. CONDITIONAL VIEW: CHARTS DASHBOARD vs RECORDS LIST */}
      {viewTab === "charts" ? (
        <IncomeChartsSection
          incomes={incomes}
          expenses={expenses}
        ...
        <>
          ...
          {/* Income Items Chronological List */}
          {sorted.length === 0 ? (
            <div
